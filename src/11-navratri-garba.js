...
export function rearrangeStage(stage, order) {
  // Your code here
  if(!stage || !Array.isArray(order) || order.length !== stage.children.length) return false;
  ...
  for(let pos of order){
    if(typeof pos !== "number" || pos < 0 || pos >= list.length || seen.has(pos)) {
      return false;
    }
    seen.add(pos);
  }
