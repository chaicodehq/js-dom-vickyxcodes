...
export function createSweetItem(name, price, category) {
  // Your code here
  if(name === undefined || price === undefined || category === undefined) return null;
  if(typeof name !== "string" || typeof category !== "string" || typeof price !== "number" || Number.isNaN(price)) return null;
...
export function buildMenuBoard(sweets) {
  ...
  if(sweets.length === 0) return menuBoardDiv;


  sweets.forEach((item) => {
    const {name, price, category} = item;
    const sweetItem = createSweetItem(name, price, category);
    if(sweetItem !== null) menuBoardDiv.appendChild(sweetItem);
  })
...
export function addSpecialBadge(sweetElement, badgeText) {
  // Your code here
  if(typeof badgeText !== "string" || badgeText === "" || sweetElement === null || sweetElement === undefined) return null;
  const span = document.createElement("span");
